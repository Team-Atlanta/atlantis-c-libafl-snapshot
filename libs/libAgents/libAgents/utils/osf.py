...
class Project:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.repo_path = None
        self._preloaded_harnesses = None  # For factory method
    ...
    def from_runner_config(
        ...
        ref_diff_path: Path = None,
    ) -> "Project":
        """
        Create a Project from runner config (bb.py output format).

        This is a simplified factory for use in the CRS runner environment where:
        - config.json contains {harness_name: source_path, ...}
        - src_path is the extracted source directory
        - No .aixcc/config.yaml is required

        Args:
            config_json_path: Path to config.json generated by bb.py
            src_path: Path to extracted source code (from project.tar.gz)
            project_name: Name of the project
            project_path: Optional path to project directory (defaults to src_path)
            ref_diff_path: Optional path to ref.diff file for delta mode

        Returns:
            Project instance configured for runner use
        """
        import json

        config_json_path = Path(config_json_path)
        src_path = Path(src_path)
        project_path = Path(project_path) if project_path else src_path

        # Load config.json: {harness_name: source_path, ...}
        with open(config_json_path) as f:
            config_data = json.load(f)

        # Convert to harnesses format expected by Project
        harnesses = {}
        for harness_name, harness_src_path in config_data.items():
            # Normalize path - replace /src with actual src_path
            normalized_path = harness_src_path
            if harness_src_path.startswith("/src/"):
                normalized_path = str(src_path / harness_src_path[5:])
            elif harness_src_path.startswith("/src-"):
                # Handle /src-config_gen style paths
                parts = harness_src_path.split("/", 2)
                if len(parts) > 2:
                    normalized_path = str(src_path / parts[2])
        ...
        instance.oss_fuzz_home = None
        instance.project_path = project_path
        instance.repo_path = src_path
        instance._preloaded_harnesses = harnesses
        instance.aixcc_config = {"harness_files": list(harnesses.values())}
        instance.harnesses = harnesses
        instance._ref_diff_path = Path(ref_diff_path) if ref_diff_path else None
    ...
    def _init_aixcc_config(self):
        ...
        # If already preloaded by factory method, return it
        if hasattr(self, "aixcc_config") and self.aixcc_config is not None:
            return self.aixcc_config
    ...
    def _init_harnesses(self):
        ...
        # If already preloaded by factory method, return it
        if hasattr(self, "_preloaded_harnesses") and self._preloaded_harnesses is not None:
            return self._preloaded_harnesses

        if not hasattr(self, "aixcc_config") or self.aixcc_config is None:
            raise ValueError(
                "AIXCC config not found (We rely on a custom config file)"
            )
    ...
    def ref_diff_path(self):
        ...
        # Check custom path first (set by from_runner_config)
        if hasattr(self, '_ref_diff_path') and self._ref_diff_path and os.path.exists(self._ref_diff_path):
            return str(self._ref_diff_path)
        # Fall back to standard .aixcc location
